#############################################################################################
######     An open-source framework for the FE modelling and optimal design of      #########
######     fiber-steered variable-stiffness composite cylinders                     #########
######                       using water strider algorithm                          #########
#############################################################################################
...
NS=134 #Number of Total narrow sections (which must be even)
L=457.200012  # L=D=18 in
D=L 
...
RP_id=myRefPoint.id 
refpoint1= (mdb.models['Model-1'].rootAssembly.referencePoints[RP_id], ) 
a = mdb.models['Model-1'].rootAssembly
...
f=open('D:\\optLaminatedComp\\proposed_T.txt')
T = []
for l in f:
	row = l.split()
	print(row)
...
n=10 # Number of narrow sections between two subsequent Ti
...
Flip_tetta=tetta[::-1] 
tetta.extend(Flip_tetta)

for i in range(NS):    
    myOrientationsList[i].extend([0,tetta[i],90,-tetta[i],-tetta[i],90,tetta[i],0])
...
########################################################################
########################################################################
file_id1=open('D:/optLaminatedComp/JOB/%d'%(jj+1+nFiles) + '/modeNumber.txt','w')
...
########################################################################
########################################################################
file_id1=open('D:/optLaminatedComp/JOB/%d'%(jj+1+nFiles) + '/bucklingMOMENT.txt','w')
...
########################################################################
########################################################################
file_id1=open('D:/optLaminatedComp/JOB/%d'%(jj+1+nFiles) + '/T.txt','w')
